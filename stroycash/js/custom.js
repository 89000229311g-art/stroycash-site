...
(function () {
  ...
  function initFloat() {
    ...
    wrap.id = 'sc-float';
    wrap.innerHTML = [
      '<a class="sc-float-btn sc-float-btn--wa" href="https://wa.me/79303346176"',
      ...
      '</a>',
      '<a class="sc-float-btn sc-float-btn--tg" href="https://t.me/stroycash_remont"',
      ...
      '<button class="sc-float-btn sc-float-btn--top" id="sc-to-top" aria-label="Наверх">',
        '<svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5"',
          ' stroke-linecap="round"><polyline points="18 15 12 9 6 15"/></svg>',
      '</button>'
    ].join('');
  ...
  function addPreconnects() {
    ['https://ws.tildacdn.com', 'https://neo.tildacdn.com'].forEach(function (url) {
  ...
  function init() {
    addPreconnects();
    // НЕ трогаем Tilda lazy loading — у Tilda свой механизм!
    initFloat();
    initReadingProgress();
    initScrollReveal();
    // Меню добавляем с задержкой (Tilda рендерит async)
    addBlogToMenu();
  }
